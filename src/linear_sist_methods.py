...
def eliminacion_gaussiana(A: np.ndarray) -> np.ndarray:
    ...
    for i in range(0, n - 1):  # loop por columna
        ...
        if p != i:
            ...
            A[i, :] = A[p, :].copy()
            A[p, :] = _aux
            logging.info(f"Matriz después del intercambio:\n{A}")  # Mostrar matriz después del intercambio
        # --- Eliminación: loop por fila
        for j in range(i + 1, n):
            m = A[j, i] / A[i, i]
            A[j, i:] = A[j, i:] - m * A[i, i:]
        logging.info(f"\nMatriz después de la eliminación en la iteración {i}:\n{A}")
    if A[n - 1, n - 1] == 0:
        raise ValueError("No existe solución única.")
...
def gauss_jordan(Ab: np.ndarray) -> np.ndarray:
    ...
    for i in range(0, n):  # loop por columna
        ...
        if p != i:
            ...
            Ab[i, :] = Ab[p, :].copy()
            Ab[p, :] = _aux
            logging.info(f"Matriz después del intercambio:\n{Ab}")  # Mostrar matriz después del intercambio
        # --- Eliminación: loop por fila
        for j in range(n):
            if i == j:
                continue
